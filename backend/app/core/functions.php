...
/* Handle Features in Products table */
function get_features(object $features): string
{
    // check for keys in the object to implode them
    if ($features->productType == 'DVD') {
        return $features->size . " MB";
    }
    if ($features->productType == 'Book') {
        return $features->weight . " KG";
    }
    if ($features->productType == 'Furniture') {
        return $features->height . "x" . $features->width . "x" . $features->length;
    }
}